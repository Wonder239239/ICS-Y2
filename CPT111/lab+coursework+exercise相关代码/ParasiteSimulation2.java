public class ParasiteSimulation2 {
	...
	public static void printarray(){
		...
		for(int i=0;i<rowCount;i++){
			for(int j=0;j<colCount;j++){
				if(j==colCount-1) System.out.println(DEFAULT_FOREST_2[i][j]+" ");
				else System.out.print(DEFAULT_FOREST_2[i][j]+" ");
			}
	...
	public static void changearray(){
		...
		for(int i=0;i<rowCount;i++){
			for(int j=0;j<colCount;j++){
				if(a[i][j]=='I'){
					if(i+1<5&&a[i+1][j]!='D') DEFAULT_FOREST_2[i+1][j]='I';
					if(i-1>=0&&a[i-1][j]!='D') DEFAULT_FOREST_2[i-1][j]='I';
					if(j-1>=0&&a[i][j-1]!='D') DEFAULT_FOREST_2[i][j-1]='I';
					if(j+1<5&&a[i][j+1]!='D') DEFAULT_FOREST_2[i][j+1]='I';
					DEFAULT_FOREST_2[i][j]='D';
				}
				
				if(a[i][j]=='D') DEFAULT_FOREST_2[i][j]='G';
			}
		}
	}
	...
	public static boolean stop(){
		...
		for(int i=0;i<rowCount;i++){
			for(int j=0;j<colCount;j++){
				if(DEFAULT_FOREST_2[i][j]=='G') m = true;
				if(DEFAULT_FOREST_2[i][j]!='G') return false;
			}
		}
	...
	public static void main(String[] args) {
		...
		char[][] forestState = DEFAULT_FOREST_2;
		...
		while(true){
			ParasiteSimulation2.changearray();
			ParasiteSimulation2.printarray();
			if(now==31) break;
			if(stop()) break;
